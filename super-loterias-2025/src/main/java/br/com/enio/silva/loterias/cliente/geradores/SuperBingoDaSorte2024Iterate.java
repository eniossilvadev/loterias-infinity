 ...
 *
 */
public class SuperBingoDaSorte2024Iterate extends Base {

	public static final int QUANTIDADE_DE_JOGOS = 1; // vezes 3
	...
	public static List<List<Integer>> execute(final int totalDeJogos, final int maxListaBase,
		...
		final int tamanhoIncluirLista = 7;
		...
		int count = 0;
		for (List<Integer> listaAtual: listaDaSorte) {

			QuinaConfigAb config = quinaConfig;

			System.out.println("Lista Atual Inicial: " + listaAtual);
			...
			ultimos = ultimos <= 10 || ultimos > ultimosResultados.size() - 1
					...
					try {
						...
						listaIncl.addAll(new ArrayList<>(listaAtual.subList(0, tamanhoIncluirLista)));
						...
						preJogos = QuadrantesQuina.generateList(qttInicialFixo, tamanhoDoJogo,
								listaExcluirFinal, listaIncluirFinal, tamanhoIncluirLista);
