...
import br.com.enio.silva.loterias.config.megasena.*;
import br.com.enio.silva.loterias.gerador.Base;
import br.com.enio.silva.loterias.util.MapUtil;

import br.com.silva.enio.loterias.commons.arquivo.ArquivoUtil;

public abstract class GerarJogosMegaLoopAb extends Base {
	...
	private static final Map<Integer, Supplier<MegaSenaConfigAb>> CONFIG_SUPPLIERS = Map.of(
			6, MegaSenaConfig6A2::new,
			7, MegaSenaConfig7::new,
			8, MegaSenaConfig8::new,
			9, MegaSenaConfig9::new,
			10, MegaSenaConfig10::new,
			11, MegaSenaConfig11::new,
			12, MegaSenaConfig12::new,
			15, MegaSenaConfig15::new
	);

	private static final Supplier<MegaSenaConfigAb>[] CONFIG_MOD_SUPPLIERS = new Supplier[] {
			MegaSenaConfig6A2::new,  // mod 0
			MegaSenaConfig6AF::new,  // mod 1
			MegaSenaConfig6A3::new,  // mod 2
			MegaSenaConfig6A2::new,  // mod 3
			MegaSenaConfig6AF::new,  // mod 4
			MegaSenaConfig6A4::new   // mod 5
	};
	 ...
	 * @return
	 */
	protected static Set<Integer> gerarExcluirAcimaMax(MegaSenaConfigAb c, int perc, int tamExcluir,
	...
	protected static Set<Integer> gerarExcluirMax(MegaSenaConfigAb config, int tamPadraoIncluir,
	...
	protected static Set<Integer> gerarExcluirMaximo(MegaSenaConfigAb config, int tamPadraoExcluir,
			List<Integer> jogosAtuaisFlat) {
	 ...
	 * @return
	 */
	protected static Set<Integer> gerarIncluirAbaixoMinimo(MegaSenaConfigAb config, int minPercent,
	...
	protected static Set<Integer> gerarIncluirMinimo(MegaSenaConfigAb config, int tamPadraoIncluir,
			List<Integer> jogosAtuaisFlat) {
	...
	protected static MegaSenaConfigAb getConfig(int count) {
		return CONFIG_MOD_SUPPLIERS[count % 6].get();
	}

	protected static MegaSenaConfigAb getConfigBySize(int size) {
		Supplier<MegaSenaConfigAb> supplier = CONFIG_SUPPLIERS.get(size);
	 ...
	 * @return
	 */
	protected static List<Integer> getFlatMega(String pathListaJogosCorrentes, int count,
			String jogosAtuais) {
	 ...
	 * @throws IOException
	 */
	protected static List<List<Integer>> getJogosAtuais(MegaSenaConfigAb config,
	 ...
	 * @throws IOException
	 */
	protected static void getJogosCorrentes(MegaSenaConfigAb config,
	...
	protected static int getMrmj(MegaSenaConfigAb config) {
	...
	protected static int getMrmj(MegaSenaConfigAb config, int mrmj) {
	...
	protected static Set<Integer> getSublistaMap(Map<Integer, Integer> map, int size) {
		Set<Integer> resultado = new HashSet<>(size); // pré-aloca capacidade

		for (Integer key : map.keySet()) { // usa keySet() ao invés de entrySet()
			resultado.add(key);
			if (resultado.size() >= size) {
				break; // break ao invés de return
			}
		}
	...
	protected SaveMegaSena saveMegaSena = new SaveMegaSena();
