...
public class GerarLotomaniaMainNew extends GerarJogosLotomaniaBase {
	...
	public static List<List<Integer>> execute(List<Integer> preJogo)
		...
		if (listaMax != null && !listaMax.isEmpty()) {
			filtro = new FiltroMaxLista(listaMax, conf.getNrosApostados() - 15);
			preJogos = filtro.filtrarListas(preJogos);
		}
		...
		for (JogoAb meuJogo : jogosLM) {
			...
			List<List<Integer>> cj = getEsquema80(meuJogo.getNumerosAsList(), mapResultado,
			        mapAtraso);
			cj = getEspelhos(cj);
