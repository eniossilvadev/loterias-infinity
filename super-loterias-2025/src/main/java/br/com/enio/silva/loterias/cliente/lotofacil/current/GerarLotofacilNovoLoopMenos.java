...
public class GerarLotofacilNovoLoopMenos extends GerarJogosLotofacil2020Ab {

	public static int ultimos = -1;

	public static final int times = 12;

	public static final int modulador = 10;
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < times; i++) {
			...
			List<Integer> elements = MapUtil.getElements(mapAtuais, modulador);
			...
			if (elements.isEmpty()) {
				...
				elements.addAll(listaRange.subList(0, modulador));
			}

			listaBaseNovosJogos.removeAll(elements);

			int completar = config.getNrosApostados() - elements.size();

			List<List<Integer>> preJogos = new ArrayList<>();

			System.out.println("completar (size): " + completar);
			System.out.println("Lista restante: " + listaBaseNovosJogos);
			System.out.println("Lista Base: " + listaBaseNovosJogos);
			System.out.println("Lista elements: " + elements);

			sleep(1);

			List<List<Integer>> pjs = CombinationUtils.gerarCombinacao(listaBaseNovosJogos,
					completar);
			...
			try {
				...
				for (List<Integer> jogo : jogos) {
					...
					System.out.println(elements.stream().map(Object::toString)
