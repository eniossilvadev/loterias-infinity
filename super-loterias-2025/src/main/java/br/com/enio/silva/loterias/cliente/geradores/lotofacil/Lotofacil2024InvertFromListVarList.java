...
public class Lotofacil2024InvertFromListVarList extends Lotofacil2024InverteAb {
	...
	public static void main(String[] args) throws IOException {
		...
		List<Integer> listaFixa = Arrays.asList(1,2,25);
		final String input = "C:\\loterias\\gerador-apostas\\lotofacil\\curr\\01_20240429_20240429223424314.txt";
		...
		for(int i = 0; i < times; i++) {
			...
			List<Integer> la = new ArrayList<>(inputList.get(0));
			String dt = DateUtils.getCurrentDefaultDateTime();

			if(listaFixa.isEmpty()) {
				throw new RuntimeException("A lista fixa não pode ser vazia");
			}

			int modd = (i % listaFixa.size()) + 1;

			Collections.shuffle(listaFixa);
			final var subListaFixa = listaFixa.subList(0, modd);

			if(subListaFixa != null && !subListaFixa.isEmpty()) {
				la.removeAll(subListaFixa);
			}
			...
			List<List<Integer>> pjs = QuadrantesLotofacil.generateList(quantidade, listSize, elements, listaBaseNovosJogos);
