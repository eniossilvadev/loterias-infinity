...
public class QuinaLoopExcluirIncluirPreJogo extends Base {
	...
	public static void main(String[] args) throws IOException {
		...
		final int totalDeJogos = 5;

		int[] ultimosJogos = { 0, 30, 50, 100, 0, 260, 0, 500, 0, 1000 };

		final List<Integer> theList = Arrays.asList();
		final List<Integer> includeRound = Arrays.asList();
		final boolean randomInclusion = true;
		...
		for (int count = 0; count < totalDeJogos; count++) {
			...
			int ultimos = ultimosJogos[(count + rand) % ultimosJogos.length];
			...
			try {
				...
				if (randomInclusion) {
					int rd = random.nextInt(config.getMaxNum()) + 1;
					listaIncl.add(rd);
				}
				...
				if (includeRound != null && !includeRound.isEmpty()) {
					listaFixos.add(includeRound.get(count % includeRound.size()));
					int seq = count * ((new Random().nextInt(80) + 1) * 100);
					while (listaFixos.size() < 2) {
						seq = seq % 80 + 1;
						if (!includeRound.contains(seq)) {
							listaFixos.add(seq);
						}
						seq++;
					}
