...
public class GerarJogosQuinaLoopQuadrante extends Base {
	...
	public static List<Integer> getNumerosDaSorte(List<List<Integer>> resultados,
		...
		return getNumerosDaSorte(resultados, listaDeJogosCorrentes, excluirLista, 160000, null);
	}

	public static List<Integer> getNumerosDaSorte(List<List<Integer>> resultados,
			List<List<Integer>> listaDeJogosCorrentes, List<Integer> excluirLista,
			final int qttInicialFixo, List<List<Integer>> listaFiltro) {

		final List<Integer> retorno = new ArrayList<>();

		QuinaConfigAb config = new QuinaConfig5(); // new QuinaConfig10();
		...
		try {
			...
			excluirLista = excluirLista != null ? new ArrayList<>(new HashSet<>(excluirLista))
					: Collections.emptyList();

			List<List<Integer>> preJogos = QuadrantesQuina.generateList(qttInicialFixo,
					config.getNrosApostados(), excluirLista, null);
