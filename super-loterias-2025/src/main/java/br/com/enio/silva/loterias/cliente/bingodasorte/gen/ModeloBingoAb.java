...
import br.com.enio.silva.loterias.cliente.bingodasorte.ModeloBingoDaSorte;

public abstract class ModeloBingoAb  implements Comparable<ModeloBingoAb>, ModeloBingoDaSorte {

	/**
	 *
	 */
	private static final long serialVersionUID = 8793182259872742653L;
	...
	public ModeloBingoAb(List<String> linha) {

		try {
			this.id = Long.valueOf(linha.get(0));
			this.nome = linha.get(1);
			this.jogos = linha.subList(2, 12).stream().map(el -> Integer.valueOf(el))
	...
	public ModeloBingoAb(long id, String nome, List<Integer> jogos) {
		super();
		this.id = id;
		this.nome = nome;
		this.jogos = jogos;
	...
	@Override
	public int compareTo(ModeloBingoAb o) {
