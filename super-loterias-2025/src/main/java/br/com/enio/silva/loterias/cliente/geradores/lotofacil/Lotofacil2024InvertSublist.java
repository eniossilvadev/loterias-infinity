...
public class Lotofacil2024InvertSublist extends Lotofacil2024InverteAb {

	private static final int modulador = 1;

	private static final int defaultSize = 15;

	private static final int quantidade = 35000;

	private static final int subListSize = 2;
	...
	public static void main(String[] args) throws IOException {
		...
		final String input = "E:\\loterias\\gerador-apostas\\lotofacil\\full.txt";
		...
		final var localSubListSize = inputList.size() > subListSize? subListSize: inputList.size();
		final var inputSubList = new ArrayList<>(inputList.subList(0, localSubListSize));

		int i = 0;

		for (List<Integer> la : inputSubList) {
			...
			try {
				...
				Map<Integer, Integer> mapInvertido = MapUtil.getMapInvertido(full, config.getMaxNum());
				...
				int numJogos = jogosLM.size() > config.getNrosJogos()? config.getNrosJogos(): jogosLM.size();
				...
				String currPairOut = CaminhoResultados.LOTOFACIL.getBasePath() + "pair/pair" + System.currentTimeMillis() + ".txt";
