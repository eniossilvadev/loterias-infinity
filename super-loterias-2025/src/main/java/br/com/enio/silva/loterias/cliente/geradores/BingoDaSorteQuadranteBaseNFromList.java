...
public class BingoDaSorteQuadranteBaseNFromList extends Base {
	...
	public static void main(String[] args) throws IOException {
		...
		final String pathIn = "E:\\loterias\\bingo-da-sorte\\curr\\x.txt";
		final String pathOut = "E:\\loterias\\bingo-da-sorte\\curr\\out.txt";

		List<List<Integer>> listaInput = ArquivoUtil.obterLinhasComoListas(pathIn);
		final int maxListaBaseDefault = 7;
		int maxListaBase = maxListaBaseDefault;
		// 03,10,21,22,52,58,60,61,71,80
		...
		Collections.shuffle(listaInput);

		for (List<Integer> listaBase: listaInput) {

			count++;

			maxListaBase = maxListaBaseDefault + count % 2;
			...
			try {
				...
				preJogos = QuadrantesQuina.generateList(qttInicialFixo, tamanhoDoJogo, excluir, listaBase, maxListaBase);
				...
				if (preJogos != null && preJogos.size() > 0) {
					...
					List<Integer> currJogo = jogosLM.get(0).getNumerosAsList();
					strOutput.append(ListaUtils.getAsString(listaBase, 2)).append(" -> ");
					strOutput.append(ListaUtils.getAsString(currJogo, 2)).append("\t");

					strOutput.append(ListaUtils.getAsString(ListaUtils.getListOneMinusListTwo(listaBase, currJogo), 2)).append("\t");
					strOutput.append(ListaUtils.getAsString(ListaUtils.getListOneMinusListTwo(currJogo, listaBase), 2)).append("\t");
					...
					ArquivoUtil.safeSave(strOutput.toString(), pathOut);
					System.err.println("See: " + pathOut);
