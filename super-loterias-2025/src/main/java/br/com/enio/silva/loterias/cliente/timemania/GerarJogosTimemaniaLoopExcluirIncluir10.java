package br.com.enio.silva.loterias.cliente.timemania;
...
import br.com.enio.silva.loterias.config.TimemaniaConfig;
import br.com.enio.silva.loterias.config.TimemaniaConfigAb;
...
import br.com.enio.silva.loterias.lotomania.JogoTimemania;

import br.com.silva.enio.loterias.commons.arquivo.ArquivoUtil;
import util.ListUtil;

public class GerarJogosTimemaniaLoopExcluirIncluir10 extends Base {
	...
	public static void main(String[] args) throws IOException {
		...
		int[] ultimosJogos = { 260, 500, 1000, 0, 50, 100, };

		// int topN = 2000;
		int qttInicialFixo = 40000;
		...
		String base = "E:\\loterias\\timemania\\config\\";
		...
		String path = CaminhoResultados.TIMEMANIA.getPath();
		...
		for (int count = 0; count < totalDeJogos; count++) {
			...
			ultimos = ultimos <= 10 || ultimos > resultados.size() - 1 ? resultados.size() - 1
			        : ultimos - 1;
			List<List<Integer>> ultimosResultados = GerarListaTimemania.getInstance()
			        .gerarArquivoResultado(path);
			...
			try {
				...
				if (preJogos != null && preJogos.size() > 0) {
					...
					List<JogoTimemania> jogosLM = new ArrayList<>();
					JogoTimemania jlm = null;
					for (List<Integer> pj : preJogos) {
						jlm = new JogoTimemania(pj);
						jogosLM.add(jlm);
					}
					...
					mc = config.getFrequencia(listaJogosCorrentes);
					ArquivoUtil.save(mc, base + "correntes.txt");
					ma = config.getFrequencia(listaDeJogos);
					ArquivoUtil.save(ma, base + "atuais.txt");
